...
//        TreeNode* end = search_node(second);
        int ans = findMax(start, first, second);
        value.push_back(ans);
    }
...
int Index::findMax(TreeNode *start, int firstkey, int secondkey){
    int max = 0;
    vector<int> ans;
    int m=0;
    while(start->key[m] <= secondkey){
        if(start->key[m] >= firstkey){
            ans.push_back(start->value[m]);
        }
        if(m == start->size-1){
            start = start->ptr[m+1];
        }
